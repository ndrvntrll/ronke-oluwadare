<?php
/*
  Plugin Name: WP Change Custom Posts Slugs
  Plugin URI: http://themeheap.com/portfolio-item/wp-change-custom-posts-slugs/
  Description: The plugin allows to can easily change slug of custom post types from WordPress admin panel.
  Version: 1.2
  Author: ThemeHeap
  Author URI: http://www.themeheap.com
 */
...
function wpccps_th_custom_post_slug_menu() {
    ...
    add_options_page( __('TH Custom Slugs', 'wpccps_th_custom_slugs' ), __('TH Custom Slugs', 'wpccps_th_custom_slugs' ), 'manage_options', 'th-wp-change-custom-post-slugs', 'wpccps_th_custom_slugs_plugin_options' );
}
...
?>
  <div class="wrap">
  <iframe src="https://themeheap.com/banner.php" width="100%" height="200"></iframe>
  <hr />
      <h2><?php _e('Change custom post type slugs', 'wpccps_th_custom_slugs'); ?></h2>
      <?php 
      $post_types = get_post_types( array( '_builtin' => false, 'publicly_queryable' => true, 'show_ui' => true ) ); 
      if( count($post_types )){
      ?>
