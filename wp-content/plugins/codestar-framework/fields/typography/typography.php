...
class CSFramework_Option_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    $theme_fonts = apply_filters( 'cs_theme_fonts', array(
      'FuturaPTBook',
      'FuturaPTMedium',
    ));
    ...
    if( is_object( $google_json ) ) {
       ...
       * Theme Font
       */
      echo '<optgroup label="'. esc_html__( 'Theme Fonts', 'cs-framework' ) .'">';
      foreach ( $theme_fonts as $theme_font ) {
        echo '<option value="'. $theme_font .'" data-type="themefonts"'. selected( $theme_font, $family_value, true ) .'>'. $theme_font .'</option>';
      }
      echo '</optgroup>';
